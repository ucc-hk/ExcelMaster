...
# html_file='it-school.html'
html_file= sys.argv[1]
#usage to generate csv report: C:\Users\USER\Desktop\Assessment\_tools\Acunetix result converter > Py -3 Acunetix_htmlToCSV_v02.py "Administration System.html"
with open(html_file, encoding='utf8', errors='ignore') as f:
    file_gf = f.readlines()

#print(type(file_gf), len(file_gf))
count=lastcountItm=lastcountItmD=num_Description=num_Impact=num_Recommendation=num_References=last_num_ReqItems=last_num_RspItems=num_affectedItems=num_totalItems=num_DInf=num_reqhdrItems=num_totalreqhdr=num_rsphdrItems=num_totalrsphdr=0
key=0        
num=1
...
#    return "\n" + Items + "\n" + requestHeaders[:1000]  + "\n" + responseHeaders[:1000]
    if header == 'Request':
        h = findingItemList[-1].get('Req_hdr','-') #default null value marked as '-'
...
#retreive all target sections class number 
for string in file_gf:
    if string.find('>Severity</td>') >= 0:
        Finding = 'Finding section begin'
#        print(Finding)
...
#         print('reqhdr '+str(count))
    ...
    if Ref_class and AffI_class and ItemD_class and reqhdr_class and rsphdr_class: break
    count+=1
end = datetime.datetime.now()
elapsed = end - start
print('time duration: ', elapsed.seconds,".",elapsed.microseconds)
############################################################

Finding = ''
count = 0
#isReqHdr=True
#Collect target sections' content
for string in file_gf:
...
##         print('reqhdr '+str(count))
    ...
    if re.search('class="'+reqhdr_class+'"', string) and isReqHdr:
        ...
        #check if continual request header section across page break and truncate
        if last_num_ReqItems == num_affectedItems:
            requestHeaders += HTMLtextMultiRow(count)+"\n\n"
        else:
            requestHeaders += str(num_reqhdrItems)+') '+HTMLtextMultiRow(count)+"\n\n"
            print(num_affectedItems)
            last_num_ReqItems=num_affectedItems
...
#     #Response Headers
    ...
    if re.search('class="'+rsphdr_class+'"', string) and not isReqHdr:
        ...
        if last_num_RspItems == num_affectedItems:
            responseHeaders += HTMLtextMultiRow(count)+"\n\n"
        else:
            responseHeaders += str(num_rsphdrItems)+') '+HTMLtextMultiRow(count)+"\n\n"
            last_num_RspItems=num_affectedItems
